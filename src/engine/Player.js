...
/**
 * Player - local player with physics, camera, and character model
 */
export class Player {
  ...
  createModel() {
    ...
    // Body colors
    const skinColor = isMathe ? 0xfdbcb4 : 0xfdbcb4;
    ...
    if (!isMathe) {
      // Kristinka gets longer hair on back
    ...
    // Role indicator (floating above head)
  ...
  update(dt, world) {
    ...
    // Animate limbs (walking animation)
  ...
  checkCollision(x, y, z, hw, world) {
    ...
    const checks = [
      ...
      [x - hw, y + PLAYER.HEIGHT, z + hw],
      [x + hw, y + PLAYER.HEIGHT, z + hw],
      [x, y + PLAYER.HEIGHT * 0.5, z], // Center body
    ];
  ...
  updateCamera(camera) {
    ...
    // Update player facing direction based on camera
